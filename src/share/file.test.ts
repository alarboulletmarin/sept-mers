...
import { canShareFile, shareFile } from './file.ts'

/*
 * Node n'implémente ni `share` ni `canShare` sur son `navigator` — pas même
 * absentes : il n'y a rien à espionner. On les y pose, puis on les retire,
 * plutôt que de les laisser fuir d'un fichier de test à l'autre.
 */
...
const exportFile = (): File =>
  new File(['{}\n'], 'sept-mers-2026-08-09.json', { type: 'application/json' })
...
describe('disponibilité de la feuille', () => {
  it('répond faux là où le navigateur ne la connaît pas', () => {
    expect(canShareFile('sept-mers.json', 'application/json')).toBe(false)
  })
  ...
  it('sonde avec un fichier vide, de mêmes nom et type', () => {
    ...
    expect(canShareFile('sept-mers-2026-08-09.json', 'application/json')).toBe(true)

    const probe = (canShare.mock.calls[0]?.[0] as ShareData).files?.[0]
    expect(probe?.name).toBe('sept-mers-2026-08-09.json')
    expect(probe?.type).toBe('application/json')
    // Vide : `canShare` ne regarde que le nom et le type, et sérialiser toutes
    // les parties pour décider d'afficher un bouton coûterait pour rien.
...
describe('envoi d un fichier', () => {
  it('atteint la feuille sans rien attendre avant', () => {
    ...
    /* Sans `await` ni tour de boucle : l'activation transitoire du clic ne
       survit pas à une attente, et Safari iOS — lui seul, ce qui rend la panne
       indétectable ailleurs — lève alors `NotAllowedError`. Cette assertion
       est tout ce qui tient la règle. */
  ...
  /* Fermer la feuille est un choix, pas une panne : l'appelant ne doit ni
     s'excuser ni compter l'envoi comme fait. */
